...
class InterfaceClockworkTriggers
{
	...
	public function runTrigger($action, $object, $user, $langs, $conf)
	{
		...
		if ($action === 'CLOCKWORK_CLOCKIN') {
			...
			$res = clockworkNotifyClockin($login, $objectId, $clockinTs, $ip, (int) $user->id);
		...
		if ($action === 'CLOCKWORK_CLOCKOUT') {
			...
			$res = clockworkNotifyClockout($login, $objectId, $clockoutTs, $netSeconds, (int) $user->id);
		...
		if ($action === 'CLOCKWORK_BREAK_START') {
			...
			$res = clockworkNotifyBreakStart($login, $shiftId, $breakStartTs, (int) $user->id);
		...
		if ($action === 'CLOCKWORK_BREAK_END') {
			...
			$res = clockworkNotifyBreakEnd($login, $shiftId, $breakEndTs, $breakSeconds, (int) $user->id);
